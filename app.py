...
def render_links():
    ...
    st.markdown(
        "<div class='warn'>以下系統的<b>實際網址尚未提供</b>"
        "（原始文件只有連結文字、沒有網址）。把網址給我、或直接在程式 <code>render_links()</code> "
        "的對應列補上即可。</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
...
| 系統 / 工具 | 用途 | 網址 |
|---|---|---|
| **廣告表** | 管理所有廣告素材與投放狀態 | 待補 |
| **換貨管理**（換貨話數、換貨表） | 換貨話術記錄與換貨表單查詢 | 待補 |
| **庫存表 / 廣達總表 / 深達總表** | 即時庫存查詢與達人總表管理 | 待補 |
| **批達人 / 評級 3.0 系統**（TikTok 系統） | 批達人申請與影片評級 | 待補 |
| **3.0 反饋話術 / 訂單查詢** | 反饋話術與達人 handle 訂單搜尋 | 待補 |
| **FAQ 問題整理** | 常見問題彙整 | 待補 |
| **達人日報 / 達人日報 SOP** | 日報填表規則與範例 | 待補 |
| **腳本拆解 / 爆款視頻例子** | 腳本拆解與爆款視頻範例 | 待補 |
| **Lark — Affiliate List / Affiliate Account** | 批達人、評級時的主要操作後台 | 待補 |
| **fastmoos** | 分析達人 GMV、均播、品類（深達建聯篩選用） | 待補 |
| **curva** | 廣達廣告表 approve（深達每日工作） | 待補 |
"""
    )
